...
def get_heightmap(color_img, depth_img, cam_intrinsics, cam_pose, workspace_limits, heightmap_resolution):
    ...
    # color_heightmap = cv2.rotate(color_heightmap, cv2.ROTATE_90_COUNTERCLOCKWISE)
    # depth_heightmap = cv2.rotate(depth_heightmap, cv2.ROTATE_90_COUNTERCLOCKWISE)
    color_heightmap = cv2.rotate(color_heightmap, cv2.ROTATE_90_CLOCKWISE)
    depth_heightmap = cv2.rotate(depth_heightmap, cv2.ROTATE_90_CLOCKWISE)
...
if __name__=='__main__':
    ...
    # workspace_limits = np.asarray([[0.3, 0.8], [-0.25, 0.25], [0., .8]])
    ...
    color_heightmap, depth_heightmap = get_heightmap(color_img, depth_img, robot.config.intrinsics, robot.config.cam_pose, workspace_limits, heightmap_resolution)
    valid_depth_heightmap = depth_heightmap.copy()
    valid_depth_heightmap[np.isnan(valid_depth_heightmap)] = 0
    plt.imshow(color_img)
    plt.savefig(os.path.join(robot.config.root, "vg/tmp", "color.png"))
    plt.imshow(depth_img)
    plt.savefig(os.path.join(robot.config.root, "vg/tmp", "depth.png"))
    plt.imshow(color_heightmap)
    plt.savefig(os.path.join(robot.config.root, "vg/tmp", "topdown_color.png"))
    plt.imshow(valid_depth_heightmap)
    plt.savefig(os.path.join(robot.config.root, "vg/tmp", "topdown_depth.png"))
